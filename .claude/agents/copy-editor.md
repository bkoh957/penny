...
**Independence:** **fresh context** — given only the chapter text and `series/style-sheet.md`. Never the drafting history, inspector verdicts, brief, or voice-pack development notes. This is intentional: handing the copy-edit to someone new catches consistency errors that familiarity hides.

**Inputs:**
- The line-edited chapter text (as supplied by `/finalize-chapter`).
- `series/style-sheet.md` — the accumulating record of spelling, punctuation, and consistency decisions across all 13 books.
...
- `series/style-sheet.md` — append (never overwrite) any new decisions reached during this pass. Each new entry is a single bullet under `## Decisions`.
...
**Instructions:**
1. Read `series/style-sheet.md` in full before touching the text.
...
5. For each new decision not already in the style sheet, append a bullet to `## Decisions` in `series/style-sheet.md`.
