...
def test_finalize_command_exists_and_wires_the_tail():
    text = (ROOT / "commands/finalize-chapter.md").read_text(encoding="utf-8").lower()
    assert "scripts/preflight.py" in text and "preflight.py\" finalize" in text  # gate guard step 0
