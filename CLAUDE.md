...
```bash
python3 -m pytest          # full suite (1370 tests); pytest.ini sets pythonpath=.
...
1. `packet_assemble.py NN MM` — deterministic, no LLM: slices this chapter's outline
   ...
   section and drop every later clue out of the obligation list `inspector-fairplay`
   grades against — that list is this section, **not** the ledger slice, which since
   the consumer split is continuity-only and reaches fairplay beside it), appends the
   continuity
